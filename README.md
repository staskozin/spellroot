...
Если публикация корректного тега прервалась до создания GitHub Release, откройте workflow `Release` на вкладке Actions, выберите `Run workflow` и укажите уже существующий тег. Повторный запуск не перемещает и не пересоздаёт тег; если release уже существует, публикация завершится ошибкой без его изменения.
